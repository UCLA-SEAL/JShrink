...
OUTPUT_LOG_DIR="${LOG_DIR}/jax_output"
...
timeout ${TIMEOUT} ${JAVA} -Xmx20g -jar ${DEBLOAT_APP} --remove-classes --maven-project ${item_dir} -T --use-cache --public-entry --main-entry --test-entry --prune-app --class-collapser --inline --remove-fields --remove-methods --log-directory "${ITEM_LOG_DIR}" --verbose 2>&1 >${temp_file}
	exit_status=$?
