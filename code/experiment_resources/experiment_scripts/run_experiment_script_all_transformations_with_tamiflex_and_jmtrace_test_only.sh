...
OUTPUT_LOG_DIR="${LOG_DIR}/all_transformations_with_tamiflex_and_jmtrace_output_test_entry_log"
...
cat ${WORK_LIST} |  while read item; do
	...
	timeout ${TIMEOUT} ${JAVA} -Xmx20g -jar ${DEBLOAT_APP} --jmtrace "${MTRACE_BUILD}" --tamiflex ${TAMIFLEX} --maven-project ${item_dir} -T --use-cache --test-entry --prune-app --class-collapser --inline --remove-fields --remove-methods --log-directory "${ITEM_LOG_DIR}" --verbose 2>&1 >${temp_file} 
	exit_status=$?
