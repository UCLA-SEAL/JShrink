...
OUTPUT_LOG_DIR="${LOG_DIR}/method_inline_with_tamiflex_and_jmtrace_output_log"
...
cat ${WORK_LIST} |  while read item; do
	...
	timeout ${TIMEOUT} ${JAVA} -Xmx20g -jar ${DEBLOAT_APP} --jmtrace "${MTRACE_BUILD}" --tamiflex ${TAMIFLEX} --maven-project ${item_dir} -T --use-cache --public-entry --main-entry --test-entry --prune-app --remove-methods --inline --log-directory "${ITEM_LOG_DIR}" --verbose 2>&1 >${temp_file} 
	exit_status=$?
